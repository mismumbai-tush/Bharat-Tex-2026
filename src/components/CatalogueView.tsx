          ...
          filteredItems.map((item, index) => (
            ...
            >
              ...
              <div className="w-24 h-24 sm:w-28 sm:h-28 flex-shrink-0 bg-slate-950 relative overflow-hidden">
                ...
                />
                <div className="absolute top-1 left-1 bg-slate-950/80 backdrop-blur-md border border-white/5 px-1 rounded text-[8px] font-mono text-amber-400 uppercase tracking-widest">
                  {item.code.split('-')[1] || 'GZ'}
                </div>
              </div>
        ...
        {selectedProduct && (
          <div className="fixed inset-0 bg-black/80 backdrop-blur-sm flex items-end justify-center z-50 p-0 sm:p-4">
            ...
            >
              ...
              <div className="p-4 bg-slate-950 border-t border-slate-850/60 flex space-x-3 flex-shrink-0">
                <button
                  onClick={() => setSelectedProduct(null)}
                  className="flex-1 py-2.5 border border-slate-800 hover:border-slate-700 bg-slate-900 rounded-xl text-slate-400 text-xs font-semibold transition-colors duration-200"
                >
                  Close Specs
                </button>
                <button
                  ...
                  className="flex-1 py-2.5 bg-gradient-to-r from-amber-500 to-amber-600 hover:from-amber-600 hover:to-amber-700 text-slate-950 text-xs font-bold rounded-xl transition-all duration-200 shadow-md shadow-amber-500/10 flex items-center justify-center space-x-1.5"
                >
